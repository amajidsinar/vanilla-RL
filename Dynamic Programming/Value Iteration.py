...
# -*- coding: utf-8 -*-
"""
Created on Tue Jun 19 16:05:44 2018
...
for i in range(1,iterations):
    ...
    for s in range(nS):
        ...
        for a in range(nA):
            ...
            q[s][a] =  policy[s][a] * transitionProbability * (reward + (gamma*vNext[s][a]))
            v[i][s] = max(q[s])
            
#policy extraction
bestActions = [np.argwhere(q[i] == np.max(q[i])) for i in range(len(q))]
