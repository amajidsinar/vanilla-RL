...
# -*- coding: utf-8 -*-
"""
Created on Thu Jul  5 21:13:52 2018
...
for e in range(episodes):
    ...
    for t in range(timesteps):
        ...
        #in the next state
        expected_sample = sum([gamma * Q[next_state][i] for i in range(nA)])
        Q[state][action] += alpha * (reward + expected_sample - Q[state][action])
        if done:
            break;
        state = next_state
